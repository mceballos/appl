...
$this->breadcrumbs=array(
		'Mensajería de usuarios'=>array('news/'),
		'Entrada'
);
else
...
<form id="message-list-form" action="<?php echo $this->createUrl($this->getId().'/'.$this->getAction()->getId()); ?>" method="post">
	...
	if($this->module->isAdmin() && $dataProvider->getItemCount() > 1) : ?>
		<div class="btn-group mailbox-checkall-buttons">
			<button class="checkall" />Clickear todo</button>
			<button class="uncheckall" />No Clickear todo</button>
		</div>
	<?php
	endif;
...
$this->widget('zii.widgets.CListView', array(
    ...
    'sortableAttributes'=>array('modified'=>'Nueva fecha'),
    'loadingCssClass'=>'mailbox-loading',
    'ajaxUpdate'=>'mailbox-list',
    'afterAjaxUpdate'=>'$.yiimailbox.updateMailbox',
    'emptyText'=>'<div style="width:100%"><h3>No hay noticias para informar.</h3></div>',
...
<?php
	...
	echo '<div class="mailbox-empty">No hay noticias para informar.</div>';
}
